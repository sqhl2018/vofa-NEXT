...
use crate::pipeline::data_plane::DataPlaneState;
...
impl AppState {
    pub fn new() -> Self {
        ...
        let can_buffer = Arc::new(Mutex::new(CanBuffer::new(50_000)));
        let can_load_stats = Arc::new(Mutex::new(CanLoadStats::new(1_000_000, 120)));
        let logic_buffer = Arc::new(Mutex::new(LogicBuffer::new(20_000)));
        let decoded_buffer = Arc::new(Mutex::new(DecodedBuffer::new(10_000)));
        let pipeline_config = Arc::new(RwLock::new(PipelineConfig::default()));

        let eval = GraphEvalState {
            graphs: graphs.clone(),
            ...
            custom_input_subscribers: custom_input_subscribers.clone(),
            filter_states: filter_states.clone(),
            decoder_states: decoder_states.clone(),
            decoder_raw_collectors: decoder_raw_collectors.clone(),
            spectrum_analyzers: spectrum_analyzers.clone(),
            spectrum_snapshot: spectrum_snapshot.clone(),
            spectrum_subscribers: spectrum_subscribers.clone(),
            ifft_states: ifft_states.clone(),
        };
        let data_plane = DataPlaneState::new(
            transport.clone(),
            eval,
            source_frames,
            can_buffer.clone(),

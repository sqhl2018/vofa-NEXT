...
use vofa_next_protocol::{CandleEngine, ProtocolEngine, CAND_CMD_RX, CAND_CMD_TX, CAND_FRAME_SIZE};
...
fn test_parse_extended_frame() {
    use vofa_next_protocol::CAND_ID_EFF;
    let mut engine = CandleEngine::new();
...
fn test_parse_rtr_frame() {
    use vofa_next_protocol::CAND_ID_RTR;
    let mut engine = CandleEngine::new();
...
fn test_encode_tx_frame() {
    use vofa_next_protocol::CAND_ID_MASK;
    let mut engine = CandleEngine::new();
    let frame = CanFrame {
        timestamp: 0,
        id: 0x123,
...
fn test_encode_extended_rtr_frame() {
    use vofa_next_protocol::{CAND_ID_EFF, CAND_ID_MASK, CAND_ID_RTR};
    let mut engine = CandleEngine::new();
    let frame = CanFrame {
        timestamp: 0,
        id: 0x12345678,
...
fn test_encode_standard_frame_full_structure() {
    ...
    let can_id_raw = u32::from_le_bytes([encoded[8], encoded[9], encoded[10], encoded[11]]);
    assert_eq!(can_id_raw, 0x123);
    assert_eq!(can_id_raw & vofa_next_protocol::CAND_ID_EFF, 0);
    assert_eq!(can_id_raw & vofa_next_protocol::CAND_ID_RTR, 0);
    assert_eq!(encoded[12], 4);
    assert_eq!(&encoded[16..20], &[0x01, 0x02, 0x03, 0x04]);
...
fn test_encode_extended_frame_eff_flag() {
    use vofa_next_protocol::{CAND_ID_EFF, CAND_ID_MASK};
    let mut engine = CandleEngine::new();
    let frame = CanFrame {
        timestamp: 0,
        id: 0x12345678,
...
fn test_split_aligned_equivalence() {
    use vofa_next_protocol::engine::FeedOutput;

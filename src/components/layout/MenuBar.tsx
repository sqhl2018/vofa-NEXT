  ...
  return [
    {
      ...
      entries: [
        ...
        { kind: 'separator' },
        { kind: 'item', id: 'quit', label: L('menuQuit'), onClick: () => void exit(0).catch(() => {}) },
      ],
    },
    {
      id: 'file',
      ...
      entries: [
        {
          ...
          onClick: () => void getCurrentWebviewWindow().minimize().catch(() => {}),
        },
        {
          kind: 'item',
          id: 'toggle-maximize',
          label: L('menuMaximize'),
          onClick: () => void getCurrentWebviewWindow().toggleMaximize().catch(() => {}),
        },
        {
          kind: 'item',
          id: 'close-window',
          label: L('menuCloseWindow'),
          onClick: () => void getCurrentWebviewWindow().close().catch(() => {}),
        },
      ],
    },
    {
      ...
      label: L('menuHelp'),
      entries: [
        { kind: 'item', id: 'docs', label: L('menuDocs'), onClick: () => void openUrl(APP_DOCS).catch(() => {}) },
        { kind: 'item', id: 'github', label: L('menuGithub'), onClick: () => void openUrl(APP_GITHUB).catch(() => {}) },
        { kind: 'separator' },
        { kind: 'item', id: 'about', label: L('menuAbout'), onClick: () => settings().openAbout() },
      ],
    },
